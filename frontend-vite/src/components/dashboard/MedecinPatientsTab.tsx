...
// Données d'exemple pour les patients du médecin
const patientsData = [
  {
    id: 1,
    nom: "Dupont",
...
interface MedecinPatientsTabProps {
  onViewDossier: (patient: any) => void
}

export function MedecinPatientsTab({ onViewDossier }: MedecinPatientsTabProps) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {patientsData.length} patient{patientsData.length > 1 ? 's' : ''} sous ma responsabilité
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-green-50 rounded-lg p-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-medium text-green-900">Âge moyen</span>
          </div>
          <p className="text-2xl font-bold text-green-900 mt-1">
            {Math.round(patientsData.reduce((sum, p) => sum + p.age, 0) / patientsData.length)} ans
          </p>
        </div>
      </div>
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                {patient.prochainRdv && (
                  <div className="text-right">
                    <p className="text-gray-500">Prochain RDV</p>
                    <p className="font-medium text-gray-900">{patient.prochainRdv}</p>
                  </div>
                )}
              </div>
